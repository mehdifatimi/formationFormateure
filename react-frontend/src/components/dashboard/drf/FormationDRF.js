...
const FormationDRF = () => {
  ...
  const handleEdit = (formation) => {
    if (validatedFormations.includes(formation.id)) {
      message.warning('Cette formation a déjà été validée et ne peut pas être modifiée');
      return;
    }
  ...
  const handleDelete = async (id) => {
    if (validatedFormations.includes(id)) {
      message.warning('Cette formation a déjà été validée et ne peut pas être supprimée');
      return;
    }
    try {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space>
          {!validatedFormations.includes(record.id) && (
            <>
              <Button
                type="primary"
                icon={<EditOutlined />}
